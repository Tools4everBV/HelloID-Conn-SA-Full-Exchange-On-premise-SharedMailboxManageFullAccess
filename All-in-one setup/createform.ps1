...
<# Begin: DataSource "Exchange-sharedmailbox-generate-table-manage-permissions-fullaccess" #>
$tmpPsScript = @'
<#----- Exchange On-Premises: Start -----#>
# Connect to Exchange
try{
...
try{
    ...
    $permissions = Get-MailboxPermission -Identity $($datasource.selectedmailbox.UserPrincipalName) | ?{($_.IsInherited -eq $false) -and -not ($_.User -like "*NT AUTHORITY*") -and ($_.AccessRights -like "*FullAccess*")} | select  @{Name="Displayname"; Expression={(Get-Recipient $_.user.ToString()).Displayname.ToString()}}, @{Name="Samaccountname"; Expression={(Get-Recipient $_.user.ToString()).sAMAccountName.ToString()}}
...
Exchange-sharedmailbox-generate-table-manage-permissions-fullaccess
'@ 
...
<# End: DataSource "Exchange-sharedmailbox-generate-table-manage-permissions-fullaccess" #>

<# Begin: DataSource "Exchange-sharedmailbox-generate-table-wildcard-fullaccess" #>
$tmpPsScript = @'
<#----- Exchange On-Premises: Start -----#>
# Connect to Exchange
try{
...
'@; 
	...
	$tmpVariables = @'
[{"name":"mailboxAlias","value":"{{form.gridMailbox.Alias}}","secret":false,"typeConstraint":"string"},{"name":"mailboxDn","value":"{{form.gridMailbox.DistinguishedName}}","secret":false,"typeConstraint":"string"},{"name":"mailboxIdentity","value":"{{form.gridMailbox.UserPrincipalName}}","secret":false,"typeConstraint":"string"},{"name":"usersToAddFullAccess","value":"{{form.fullaccessList.leftToRight.toJsonString}}","secret":false,"typeConstraint":"string"},{"name":"usersToRemoveFullAccess","value":"{{form.fullaccessList.rightToLeft.toJsonString}}","secret":false,"typeConstraint":"string"}]
'@ 

	$delegatedFormTaskGuid = [PSCustomObject]@{} 
$delegatedFormTaskName = @'
Exchange on-premise-manage-full-access-shared-mailbox-permissions
'@
